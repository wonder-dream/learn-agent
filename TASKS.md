...
| M1 Day 3-10 | Easy 为主 | 每天 2 道，优先 Easy，允许少量 Medium |
| M1 Day 11-15 | Easy + 少量 Medium | 每天 2 道 |
| M2 Day 16-35 | Medium 为主 | 每天 3 道，穿插复习 |
| M3 Day 36-40 | 专项突击 | 每天 6-10 道（Hard 题可减量，保理解不保数量） |

## 设计原则

- Transformer 不在 M1 死记硬背，移到 M3 和八股一起学（避免"学了不用就忘"）
- Schema / Prompt / Server 只写一次，后续复用（M1 定义 → M2 使用）
- LangGraph 学完立即在项目中使用（Day 12-13 学 → Day 21 用）
- MCP 是工程实践（给项目加 MCP Server），RAG 是微项目（有代码产出）
- 每日自检机制：白纸默写 3 个关键点，0-3 分决定次日推进还是重做
